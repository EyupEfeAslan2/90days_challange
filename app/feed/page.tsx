...
export default async function FeedPage() {
  ...
  // SORGIDAKİ YORUM SATIRINI KALDIRDIM, ARTIK ÇALIŞACAK
  ...
  if (error) {
    console.error("Feed SQL Hatası:", error)
  }
    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-8 pt-32 pb-20">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-end justify-between mb-8 border-b border-gray-800 pb-6">
            ...
            <Link href="/feed/create" className="bg-red-700 text-white px-5 py-2.5 rounded-lg font-bold text-sm hover:bg-red-600 transition shadow-[0_0_15px_-5px_#dc2626]">+ KONU AÇ</Link>
        </div>
                    ...
                    <Link href={`/feed/${post.id}`} className="block bg-[#0a0a0a] border border-gray-800 rounded-xl p-6 hover:border-gray-600 transition group cursor-pointer">
                        <div className="flex justify-between items-start mb-2 pr-10">
                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                <span className="font-bold text-red-500">@{authorName || 'Bilinmeyen Asker'}</span>
                                <span>•</span>
                                <span>{new Date(post.created_at).toLocaleDateString('tr-TR')}</span>
                            </div>
                        </div>
                        <h2 className="text-xl font-bold text-white mb-2 group-hover:text-red-500 transition">{post.title}</h2>
