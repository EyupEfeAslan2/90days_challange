...
import { addComment, votePost, deletePost } from './actions' // <--- deletePost eklendi
...
export default async function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !post) {
    return <div className="text-white pt-32 text-center">Konu bulunamadı.</div>
  }
  ...
  // 3. Oyları Hesapla
  const votes = post.forum_likes || []
  const upvotes = votes.filter((v: any) => v.vote_type === 'up').length
  
  // Benim oyum ne?
  const myVote = votes.find((v: any) => v.user_id === user.id)?.vote_type
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-8 pt-32 pb-20">
      <div className="max-w-3xl mx-auto">
        ...
        <Link href="/feed" className="text-gray-500 hover:text-white mb-6 inline-flex items-center gap-2 transition">
            <span>←</span> Akışa Dön
        </Link>
        ...
        <div className="bg-[#0a0a0a] border border-gray-800 rounded-xl p-6 mb-6">
           ...
           <div className="flex items-center gap-3 mb-6 border-b border-gray-800 pb-4">
              ...
              <div>
                 <div className="font-bold text-white text-lg">@{authorName || 'Bilinmeyen'}</div>
                 <div className="text-xs text-gray-500 font-mono">{new Date(post.created_at).toLocaleDateString('tr-TR')}</div>
              </div>
           </div>
             ...
             <div key={comment.id} className="flex gap-4 p-4 rounded-xl hover:bg-white/5 transition duration-300">
                ...
                <div className="space-y-1">
                   <div className="flex items-center gap-2">
                      <span className="font-bold text-white text-sm">@{comment.profiles?.username || 'Bilinmeyen'}</span>
                      <span className="text-[10px] text-gray-600">{new Date(comment.created_at).toLocaleTimeString('tr-TR', {hour: '2-digit', minute:'2-digit'})}</span>
                   </div>
           ...
           <form action={addComment} className="mt-8 flex gap-4">
              ...
              <div className="flex-1 space-y-3">
                  ...
                  <div className="flex justify-end">
                    <button type="submit" className="bg-blue-600 text-white px-5 py-2 rounded-full font-bold text-sm hover:bg-blue-500 transition">
                        Yorum Yap
                    </button>
                  </div>
              </div>
